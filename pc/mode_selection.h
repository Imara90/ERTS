...
void mode_selection(int *sel_mode,int *ae, int lift) {
    //printf(" \n SELECTED MODE: %d", *sel_mode);
    
    if( *sel_mode == MODE_SAFE || *sel_mode == MODE_ABORT || (prev_mode == MODE_SAFE && (*ae == 0 && *(ae+1) == 0 && *(ae+2) == 0 && *(ae+3) == 0 && lift == 0)) && *sel_mode != MODE_PANIC && ((*sel_mode != MODE_FULL_CONTROL && *sel_mode != MODE_YAW_CONTROL) || (TELEMETRY_FLAG & 0x01) == TRUE) || (*sel_mode == MODE_PANIC && prev_mode != MODE_SAFE) || (*sel_mode == MODE_P && (prev_mode == MODE_YAW_CONTROL || prev_mode == MODE_FULL_CONTROL)) || (prev_mode == MODE_P && *sel_mode == last_control_mode) ) 
    {   
        *sel_mode = *sel_mode;
